...
import {listDept, getDept} from "@/api/api/dept"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        deptId: [
          { required: true, message: "معرف القسم مطلوب", trigger: "blur" }
        ],
        leaveNature: [
  ...
  methods: {
    ...
    getDeptName(id) {
      const dept = this.deptOptions.find(d => d.deptId === id);
      return dept ? dept.deptName : 'غير معروف';
    },
